...
class RoundtripModel(object):
    def __init__(self, g_net, h_net, dx_net, dy_net, x_sampler, y_sampler, data, pool, batch_size, alpha, beta ,df, is_train):
        ...
        #weight clipping
        #对神经网络 dx_net 和 dy_net 的所有可训练变量进行 梯度裁剪（Gradient Clipping），限制它们的取值范围，防止梯度爆炸或梯度消失。
    ...
    def train(self, epochs, cv_epoch, patience):
        ...
        for epoch in range(epochs):
            ...
            #学习率为 0.0002 的 Adam 优化器 用于反向传播和更新模型参数。当在连续 10 个 epoch 的验证集中的平均对数似然没有改进时，我们停止模型训练
            lr = 2e-4 #if epoch < epochs/2 else 1e-4 #*float(epochs-epoch)/float(epochs-epochs/2)
            batch_idxs = len(data_y_train) // self.batch_size #共有多少batch
            for idx in range(batch_idxs):
                ...
                counter+=1
            if epoch == cv_epoch:
                global best_sd, best_scale, f_val
                if use_cv:
            ...
            if epoch >= cv_epoch:
                ...
                #wait=0
                #0.0
                py_est_test = self.estimate_py_with_IS(data_y_test, epoch, sd_y=best_sd, scale=best_scale,
                                                       sample_size=2000, log=True)
    ...
    def estimate_py_with_IS(self,y_points,epoch,sd_y=0.5,scale=0.5,sample_size=40000,bs=1024,log=True,save=True):
        ...
        #multi_process to parallel the program
        def program_paral(func,param_list):
            ...
            pool.join()
            return results
        #计算 𝑝(𝑦∣𝑥)p(y∣x)： 这是计算给定 𝑥 时，目标数据点 y 的条件概率。假设 y 的分布为高斯分布。
        ...
        #计算似然比 计算给定样本 x 时的似然比，q(x) 是从 t-分布采样得到的分布
        ...
        def sample_from_qx(x_point):
            ...
            #生成一个卡方分布（Chi-squared distribution）的随机样本，self.df 表示自由度（degrees of freedom），sample_size 表示生成样本的数量。
            z1 = np.random.chisquare(self.df, sample_size)/self.df
            #生成一个多元正态分布（Multivariate Normal distribution）的随机样本，均值为 np.zeros(self.x_dim)（全零向量），协方差矩阵为 S。
            z2 = np.random.multivariate_normal(np.zeros(self.x_dim),S,(sample_size,))
            #通过将每个样本的 z2 除以对应的 np.sqrt(z1) 值，从而对每个样本进行缩放，调整其标准差。[:, None] 用于将 np.sqrt(z1) 转换成列向量，以便在广播时与 z2 进行按元素的除法。
