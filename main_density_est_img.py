...
class RoundtripModel(object):
    def __init__(self, g_net, h_net, dx_net, dy_net, x_sampler, y_sampler, data, pool, batch_size, nb_classes, alpha,
                 beta, df, is_train):
        self.data = data
        self.g_net = g_net
        self.h_net = h_net
        ...
        # print("Input tensor shape:",self.y_.shape)
        ...
        self.graph_dir = 'graph/density_est_{}_{}_x_dim={}_y_dim={}_alpha={}_beta={}'.format(self.timestamp, self.data,
        ...
        self.save_dir = 'data/density_est_{}_{}_x_dim={}_y_dim={}_alpha={}_beta={}'.format(self.timestamp, self.data,
                                                                                           self.x_dim, self.y_dim,
                                                                                           self.alpha, self.beta)
        ...
        for epoch in range(epochs):
            ...
            for idx in range(batch_idxs):
                ...
                with tf.control_dependencies(update_ops):
                    fake_bx, fake_by, g_summary, _ = self.sess.run(
                        [self.x_, self.y_, self.g_merged_summary, self.g_h_optim],
                        feed_dict={
                            self.x: bx,
                            self.x_onehot: by_onehot,
                            self.y: by,
                            self.lr: lr
                        }
                    )
                ...
                with tf.control_dependencies(update_ops):
                    d_summary, _ = self.sess.run(
                        [self.d_merged_summary, self.d_optim],
                        feed_dict={
                            self.x: bx,
                            self.x_onehot: by_onehot,
                            self.y: by,
                ...
                if counter % 100 == 0:
                    ...
                    dx_loss, dy_loss, d_loss = self.sess.run([self.dx_loss, self.dy_loss, self.d_loss], \
                                                             feed_dict={self.x: bx, self.x_onehot: by_onehot,
                                                                        self.y: by, self.fake_x: fake_bx,
                                                                        self.fake_y: fake_by})
    ...
    def estimate_py_with_IS(self, y_points, onehot_label, epoch, sd_y=0.1, scale=0.01, sample_size=40000, bs=1024,
                            log=True, save=True):
        np.random.seed(1024)
        ...
        def py_given_x(zip_list):
            ...
            if log:
                return -self.y_dim * np.log((np.sqrt(2 * np.pi) * sd_y)) - (
                    np.sum((y_point - y_points_) ** 2, axis=1)) / (2. * sd_y ** 2)
            else:
                return 1. / ((np.sqrt(2 * np.pi) * sd_y) ** self.y_dim) * np.exp(
                    -(np.sum((y_point - y_points_) ** 2, axis=1)) / (2. * sd_y ** 2))
        ...
        for b in range(int(np.ceil(N * 1.0 / bs))):
            ...
            batch_py_given_x_list = program_paral(py_given_x, zip(batch_x_points_sample_list, batch_label_sample_list,
                                                                  batch_y_points))
            batch_w_likelihood_ratio_list = program_paral(w_likelihood_ratio,
                                                          zip(batch_x_points_, batch_x_points_sample_list))
        ...
        if log:
            ...
            py_est = np.array(
                [np.log(np.sum(np.exp(item[0] - item[0][item[1]]))) - np.log(sample_size) + item[0][item[1]] for item in
                 zip(py_list, max_idx_list)])
        else:
            py_list = map(lambda x, y: x * y, py_given_x_list, w_likelihood_ratio_list)
    ...
    def estimate_py_with_CF(self, y_points, onehot_label, epoch, sd_y=0.1, log=True, save=True):
        ...
        c_y = map(lambda x, y, z, w: np.sum(x ** 2) + y / sd_y ** 2 - z.T.dot(w).dot(z), x_points_, rt_error, mu,
                  Sigma_inv)
        if log:
            py_est = map(
                lambda x, y: -self.y_dim * np.log(np.sqrt(2 * np.pi) * sd_y) + 0.5 * np.log(np.linalg.det(x)) - 0.5 * y,
                Sigma, c_y)
        else:
            py_est = map(lambda x, y: 1. / (np.sqrt(2 * np.pi) * sd_y) ** self.y_dim * sd_y ** self.y_dim * np.sqrt(
                np.linalg.det(x)) * np.exp(-0.5 * y), Sigma, c_y)
        if save:
            np.save('%s/py_est_at_epoch%d.npy' % (self.save_dir, epoch), py_est)
        return py_est
    ...
    def save(self, epoch):
        ...
        checkpoint_dir = 'checkpoint/density_est_{}_{}_x_dim={}_y_dim={}_alpha={}_beta={}'.format(self.timestamp,
                                                                                                  self.data, self.x_dim,
                                                                                                  self.y_dim,
                                                                                                  self.alpha, self.beta)
    ...
    def load(self, pre_trained=False, timestamp='', epoch=999):
        ...
        if pre_trained == True:
            ...
            if timestamp == '':
                ...
                checkpoint_dir = 'checkpoint/density_est_{}_{}_x_dim={}_y_dim={}_alpha={}_beta={}'.format(timestamp,
                                                                                                          self.data,
                                                                                                          self.x_dim,
                                                                                                          self.y_dim,
                                                                                                          self.alpha,
                ...
                ckpt_path = os.path.join(checkpoint_dir, 'model.ckpt-%d' % epoch)
                ...
                for var in tf.global_variables():
                    ...
                    if not any(
                            x in var_name for x in ['Adam', 'Momentum', 'beta1_power', 'beta2_power', 'global_step']):
                        vars_to_load.append(var)

                # 创建只包含模型变量的 Saver
                saver = tf.train.Saver(var_list=vars_to_load)  # 移除了 allow_partial

                try:
                    saver.restore(self.sess, ckpt_path)
                    print("成功加载模型权重（跳过优化器状态）")
                except tf.errors.NotFoundError as e:
                    print("部分变量加载失败:", str(e))
                    # 使用更宽松的方式处理缺失变量
                    reader = tf.train.NewCheckpointReader(ckpt_path)
                    var_shape_map = reader.get_variable_to_shape_map()

                    # 构建变量名到变量的映射
                    var_dict = {var.name.split(':')[0]: var for var in vars_to_load}

                    # 找出检查点和当前图中都存在的变量
                    available_vars = {}
                    for var_name in var_shape_map:
                        if var_name in var_dict:
                            if var_shape_map[var_name] == var_dict[var_name].shape.as_list():
                                available_vars[var_name] = var_dict[var_name]

                    if available_vars:
                        partial_saver = tf.train.Saver(var_list=available_vars)
                        partial_saver.restore(self.sess, ckpt_path)
                        print(f"成功加载了 {len(available_vars)}/{len(vars_to_load)} 个变量")
                    else:
                        print("没有找到可加载的变量")
                        raise
...
if __name__ == '__main__':
    ...
    if args.train:
        g_net = model.Generator_img(input_dim=x_dim, output_dim=y_dim, name='g_net', nb_layers=2, nb_units=256,
                                    dataset=data, is_training=True)
    else:
        g_net = model.Generator_img(input_dim=x_dim, output_dim=y_dim, name='g_net', nb_layers=2, nb_units=256,
                                    dataset=data, is_training=False)
   ...
   # xs = util.Mixture_sampler(nb_classes=nb_classes, N=10000, dim=x_dim, sd=1)
    ...
    RTM = RoundtripModel(g_net, h_net, dx_net, dy_net, xs, ys, data, pool, batch_size, nb_classes, alpha, beta, df,
                         is_train)
