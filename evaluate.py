...
def visualization_2d(x1_min, x1_max, x2_min, x2_max, sd_y, scale, n=100):
    #v1, v2 是网格的 x1 轴和 x2 轴坐标矩阵。data_grid 是 (n², 2) 形状的 所有网格点坐标，每行是 (x1, x2)
    v1, v2, data_grid = create_2d_grid_data(x1_min, x1_max, x2_min, x2_max,n)
    #使用网格上的所有数据点来计算每个点的概率密度（py），并假设 py 返回的密度值是一个一维数组（n² 个点）。
    #然后用 reshape 将其转化为一个 n x n 的二维数组，用于绘制图像。
...
cmap='Blues', alpha=0.9)
    ...
    plt.close()
#主要用于 评估异常检测模型的效果，
#使用了 One-Class SVM、Isolation Forest 和 Roundtrip 三种方法，并计算 Precision@K（前 K 精度）
def odd_evaluate():
    ...
    #Isolation Forest
    #算法通过对样本点的孤立来检测异常值。具体来说，该算法利用一种名为孤立树iTree的二叉搜索树结构来孤立样本。
    #由于异常值的数量较少且与大部分样本的疏离性，
    #因此，异常值会被更早孤立出来，也即异常值会距离iTree的根节点更近，而正常值则会距离根节点有更远的距离
    clf = IsolationForest()
    clf.fit(X_train)
...
def precision_at_K(score, label_test):
    ...
    return precision
#计算后验概率，并基于贝叶斯决策规则进行分类，最终 计算测试集的分类准确率。
def posterior_bayes():
...
#用于 可视化 MNIST 或 CIFAR-10 数据集的样本，并按类别排序
def visualize_img():
    if data == "mnist":
...
#从给定的路径中解析出特定的实验信息，包括 x_dim、y_dim 和 timestamp
